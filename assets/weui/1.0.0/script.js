...
$(".weui-tabbar__item").on("click", function tabbar_switcher (){
  $(".weui-tabbar__item").removeClass("weui-bar__item--on");
  $(this).addClass("weui-bar__item--on");
  $.toast($(this).children(".weui-tabbar__label").text(), "text");
});


$(".nav-btn-left").on("click", function(){
  $.prompt("请输入自定义消息", function(text) {
    $.toast("你输入的是："+text, "text");
  }, function() {
    $.toast("你点击了取消", "text");
  });
});
